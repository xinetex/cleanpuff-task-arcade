...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: "Neon DB tables created and seeded successfully" });
  } catch (err: any) {
    return res.status(500).json({ error: err.message || "Failed to initialize Neon DB" });
  }
}
